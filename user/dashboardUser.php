<?php
    include '../config.php';
    $query_katalog = mysqli_query($conn, "SELECT alat.*, kategori.kategori 
                                          FROM alat 
    ...
    $query_hero = mysqli_query($conn, "SELECT gambar, nama_alat FROM alat WHERE gambar != '' AND gambar IS NOT NULL ORDER BY RAND() LIMIT 1");
            ...
            <div class="relative">
                    <div class="bg-white p-4 rounded-[2.5rem] cartoon-border cartoon-shadow rotate-3">
                        <div class="rounded-[2rem] aspect-[4/3] overflow-hidden border-4 border-black">
                            <?php if ($hero && !empty($hero['gambar'])): ?>
                                <img src="../uploads/<?php echo htmlspecialchars($hero['gambar']); ?>"
                                     alt="<?php echo htmlspecialchars($hero['nama_alat']); ?>"
    ...
    <section id="tentang" class="py-32 bg-white cartoon-border border-x-0 relative z-10">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
                <div class="group p-8 rounded-[2.5rem] bg-aksen cartoon-border cartoon-shadow transition-all">
                    <div
                    ...
                    <h3 class="text-xl font-black text-slate-900 mb-4 uppercase italic">Proses Serba Otomatis</h3>
                    <p class="text-slate-900 hover:text-aksen font-bold">Mulai dari cek stok hingga hitung denda,
                        semuanya diatur oleh
                        sistem pintar kami secara real-time.</p>
                </div>
                <div class="group p-8 rounded-[2.5rem] bg-yellow-50 cartoon-border cartoon-shadow transition-all">
                    <div
                        class="w-16 h-16 bg-white cartoon-border rounded-2xl flex items-center justify-center mb-8 shadow-[4px_4px_0px_0px_#000]">
                        <i data-lucide="shield-check" class="text-primary w-8 h-8"></i>
                    </div>
                    <h3 class="text-xl font-black text-slate-900 mb-4 uppercase italic">Jaminan Alat Berkualitas</h3>
                ...
                <div class="group p-8 rounded-[2.5rem] bg-aksen cartoon-border cartoon-shadow transition-all">
                    ...
                    <h3 class="text-xl font-black text-slate-900 mb-4 uppercase italic">Layanan 24/7</h3>
    ...
    <section id="layanan" class="py-8 px-4 bg-[#F1F5F9] relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            <div class="text-center max-w-2xl mx-auto mb-16">
                ...
                <p class="text-slate-500 mt-6 font-bold italic"> 5 langkah sederhana untuk menyewa alat produksi
                    Kami.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="bg-primary cartoon-border cartoon-shadow rounded-[3rem] p-12 overflow-hidden relative">
                <div class="relative z-10 grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        <span
                            class="text-aksen font-black uppercase tracking-widest text-xs italic bg-white cartoon-border px-4 py-1.5 rounded-full">Layanan
                            Utama</span>
                        <h2 class="text-4xl font-black text-white mt-6 leading-tight uppercase">Siap Memulai
                            Produksi?<br>Pinjam Alat Sekarang.</h2>
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="flex overflow-x-auto gap-6 pb-8 snap-x no-scrollbar">
                ...
                <?php if (mysqli_num_rows($query_katalog) > 0): ?>
                    <?php while ($alat = mysqli_fetch_assoc($query_katalog)): ?>
                    <div
                        class="snap-start shrink-0 w-[260px] md:w-[300px] bg-slate-50 cartoon-border cartoon-shadow-sm rounded-[2rem] p-4 hover:bg-yellow-50 transition-all group">
                        <div
                            ...
                            <?php if (!empty($alat['gambar'])): ?>
                                <img src="../uploads/<?php echo htmlspecialchars($alat['gambar']); ?>"
                        ...
                        <div class="space-y-1">
                            <p class="text-aksen font-black text-[9px] uppercase italic">
                                <?php echo htmlspecialchars($alat['kategori'] ?? 'Umum'); ?>
                            </p>
                            <h3 class="font-black text-base text-slate-900 uppercase leading-tight">
                                <?php echo htmlspecialchars($alat['nama_alat']); ?>
                            </h3>
                            <div class="flex items-baseline gap-1 mt-1 mb-3">
                                <span class="text-xl font-black text-primary italic">Rp<?php
                                    $h = $alat['harga_sewa'];
                                    if ($h >= 1000000) echo number_format($h/1000000, 1, '.', '').'jt';
                                    elseif ($h >= 1000) echo number_format($h/1000, 0, '.', '').'k';
                                    else echo $h;
                                ?></span>
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex flex-col md:flex-row gap-16 items-center">
                ...
                <div class="w-full md:w-1/2 space-y-10">
                    <div>
                        <h2 class="text-4xl font-black text-slate-900 uppercase">Prosedur Pengembalian</h2>
    ...
    <footer id="alamat" class="bg-primary text-white pt-24 pb-12 cartoon-border border-x-0 border-b-0">
        <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-4 gap-12 text-center md:text-left">
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm font-bold italic">
                    <li>
    ...
    <script>
        ...
        window.addEventListener('scroll', () => {
            ...
            navLinks.forEach(link => {
                ...
                // Mencocokkan href (#tentang, #beranda, dll)
                if (link.getAttribute('href').includes(current)) {
                    link.classList.add('active');
                }
            });

            // Efek Navbar menyusut saat scroll
